...
//Read input.txt file and store all info in a struct of type InputData.
int ReadInputFile(std::string infile, InputData *indata){
    ...
    if (!inFile.is_open()){
        ...
        WriteInputFileText();
        std::cout << "Could not file input.txt. A new standard input file has been generated to work with. Please use that file to change parameters of the system" << std::endl;
        return 1;
    }
    int lineNum = 0;
    std::string line;
    ...
    while (std::getline(inFile, line))
    {
        ...
        iss >> v1 >> v2;
        try{
            if (v1=="chainsize:")       indata->chainsize = int(v2);
...
int CheckData(InputData *indata){
    ...
    else if (indata->density>1.9) {
        std::cout << "ERROR: Value of density must be lower than " << 1.9 << std::endl;
        return 1;
    }
...
std::string transformDouble(double num,int n, int m) {
    // Ensure the number is positive (handle negatives if needed)
    num = std::fabs(num);

    // Extract the integer and fractional parts
    int integerPart = static_cast<int>(num);
    int fractionalPart = static_cast<int>((num - integerPart) * pow(10,m));

    // Create a stringstream to format the output
    std::ostringstream result;

    // Format the integer part to 3 digits
    result << std::setfill('0') << std::setw(n) << integerPart;

    // Add the separator 'p'
    result << "p";

    // Format the fractional part to 2 digits
    result << std::setfill('0') << std::setw(m) << fractionalPart;
